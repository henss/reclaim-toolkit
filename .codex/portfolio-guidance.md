...
- When a repo exposes Agent Atlas surfaces or rollout guidance, prefer read-only Atlas MCP tools when configured. If MCP is unavailable, run `atlas context-pack` for broad or multi-seam tasks and `atlas resolve-path` for file-specific tasks before broad repository search; read `docs/agents/atlas.md` as the fallback orientation surface. Run `atlas doctor --path .` when Atlas setup looks stale or broken. After non-trivial work, write an `atlas usage-note` receipt when the CLI is available, naming selected entities, files, tests, whether broad search was still needed, and missing or misleading cards; until OPS-2245 is fixed, write receipts outside `.agent-atlas` with `--out .runtime/agent-atlas/usage/<task-slug>.yaml` so validation does not parse receipts as entity cards. Do not create or expand atlas metadata speculatively; add it only when a concrete task, measured navigation waste, or cross-repo ambiguity demonstrates the need. Significant Agent Atlas bugs, gaps, or feature requests should become Linear issues through the approved Linear path instead of remaining only in chat, receipts, or session notes.
