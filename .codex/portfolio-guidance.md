...
- Treat useful agent judgment, recommendations, planning, venture, creative, quality-of-life, public, distribution, tool, or small ad tests as normal work when downside is recoverable or a scoped autonomy envelope records the cap, stop condition, measurement, and readout. Stop for dangerous, irreversible, authority-changing, life-changing, or hard-to-recover downside.
...
- When a repo exposes Agent Atlas surfaces or rollout guidance, prefer read-only Atlas MCP tools when configured. If MCP is unavailable, run repo-advertised `atlas context-pack`/`atlas resolve-path` scripts or CLI commands for broad, multi-seam, or file-specific orientation before broad repository search; read `docs/agents/atlas.md` as the fallback orientation surface when tooling is unavailable or stale. Run `atlas doctor --path .` when Atlas setup looks stale or broken. If Atlas remains unavailable after a limited setup check, record the setup gap in the outcome or receipt path and continue with scoped local orientation. After non-trivial work, write an `atlas usage-note` receipt when the CLI is available, naming selected entities, files, tests, whether broad search was still needed, and missing or misleading cards; until OPS-2245 is fixed, write receipts outside `.agent-atlas` with `--out .runtime/agent-atlas/usage/<task-slug>.yaml` so validation does not parse receipts as entity cards. Do not create or expand atlas metadata speculatively; add it only when a concrete task, measured navigation waste, or cross-repo ambiguity demonstrates the need. Significant Agent Atlas bugs, gaps, or feature requests should become Linear issues through the approved Linear path instead of remaining only in chat, receipts, or session notes.
