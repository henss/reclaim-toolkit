...
- When a repo exposes Agent Atlas surfaces, read `docs/agents/atlas.md` for broad orientation, use `atlas context-pack` for broad or multi-seam tasks, and use `atlas resolve-path` for file-specific tasks before broad repository search. Do not create or expand atlas metadata speculatively; add it only when a concrete task, measured navigation waste, or cross-repo ambiguity demonstrates the need.
