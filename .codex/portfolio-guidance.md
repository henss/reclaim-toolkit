...
- When a repo exposes Agent Atlas surfaces such as `.agent-atlas`, `docs/agents/atlas.md`, or `pnpm atlas ...`, use them before broad repository search. Do not create or expand atlas metadata speculatively; add it only when a concrete task, measured navigation waste, or cross-repo ambiguity demonstrates the need.
