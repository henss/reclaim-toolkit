...
| --- | --- | --- |
| Local preview | `reclaim:onboarding`, `reclaim:tasks:preview-create`, `reclaim:tasks:preview-update`, `reclaim:tasks:preview-retarget`, `reclaim:habits:preview-create`, `reclaim:focus:preview-create`, `reclaim:buffers:preview-create`, `reclaim:buffers:preview-template`, `reclaim:meetings:preview-recurring-reschedule`, `reclaim:meetings-hours:preview-inspect`, `reclaim:meetings-hours:preview-switch`, `reclaim:hours-config:preview-audit`, `reclaim:hours-config:preview-diff`, `reclaim:account-audit:preview-inspect`, `reclaim:account-audit:preview-drift`, `reclaim:time-policies:explain-conflicts`, `reclaim:support:bundle`, `reclaim:demo:mock-api`, `reclaim:config:status` | No live Reclaim writes. |
| Authenticated read | `reclaim:health`, `reclaim:time-policies:list`, `reclaim:tasks:list`, `reclaim:tasks:filter`, `reclaim:tasks:export`, `reclaim:tasks:validate-write-receipts`, `reclaim:tasks:inspect-duplicates`, `reclaim:meetings-hours:inspect`, `reclaim:hours-config:audit`, `reclaim:account-audit:inspect` | Reads account data through the configured Reclaim API key. |
| Confirmed write | `reclaim:tasks:create`, `reclaim:tasks:update`, `reclaim:tasks:retarget`, `reclaim:tasks:cleanup-duplicates` | Requires an explicit confirmation flag before live writes. |

`reclaim:tasks:create`, `reclaim:tasks:update`, and `reclaim:tasks:retarget` refuse to write unless `--confirm-write` is present. `reclaim:tasks:cleanup-duplicates` refuses to delete unless `--confirm-reviewed-delete` is present. Confirmed write results include `writeReceipts` with the operation, task id, confirmation timestamp, and manual rollback hint for audit.

Task, Habit, Focus, and Buffer preview commands now include a top-level `previewReceipt` with the operation name, preview timestamp, readiness status, readiness gate, and rollback hint. Habit, Focus, and Buffer still include `writeSafety: "preview_only"`, while task create and update previews keep the exact request payloads that would be sent by later confirmed task writes without contacting Reclaim. When task, Focus, or Buffer preview inputs include synthetic policy context, each preview item also includes a `timePolicyExplanation` object with selected-policy reasoning and fit/conflict details. Focus preview also accepts an optional synthetic `currentFocusBlocks` baseline and then emits per-block `planDiff` receipts plus top-level `removedFocusBlocks` and `planDiffSummary` fields so callers can inspect create-vs-update-vs-remove planning diffs locally. `reclaim:buffers:preview-rule` includes synthetic `mockResponse` metadata plus `previewReceipt.diffLines` and `previewReceipt.diffSummary` so callers can inspect create-vs-update diffs without contacting Reclaim; when the same fixture also includes synthetic `timeSchemes` and bounded windows, the receipt adds `protectedTimeWindowDiff` with `+`, `~`, and `-` day lines for protected-window fit. `reclaim:buffers:preview-template` also includes synthetic `mockResponse` and `previewReceipt` fields so template evaluation can inspect receipt-style output without contacting Reclaim.
...
The Hours Config audit includes only hours-policy coverage, window counts, timezone spread, and weekday coverage, plus `readSafety: "read_only"`, so downstream consumers can review configuration posture without relying on policy ids or policy titles.
The Hours Config diff digest keeps the same read-only boundary. It accepts two synthetic hours-config snapshots, preserves only source handles in the comparison envelope, and classifies drift without replaying policy ids or policy titles.
...
- `reclaim:tasks:preview-create` returns the prepared task payloads plus a `previewReceipt` showing that the surface is ready for a later confirmed write. When the input also includes synthetic policy context, each task row carries a `timePolicyExplanation`.
- `reclaim:tasks:preview-update` returns prepared task patch payloads, optional synthetic current-task change summaries, and a `previewReceipt` showing that the surface is ready for a later confirmed update.
- `reclaim:tasks:preview-retarget` returns date-shifted task patch payloads from synthetic current-task snapshots, plus the anchor delta and a `previewReceipt` showing that the surface is ready for a later confirmed retarget.
...
- `reclaim:habits:preview-create`, `reclaim:focus:preview-create`, and `reclaim:buffers:preview-create` return preview payloads plus a `previewReceipt` that explains why each surface remains preview-only. Focus and Buffer previews also emit per-item `timePolicyExplanation` fields when the input fixture includes synthetic policy context, and Focus preview can add `planDiff` fields when a synthetic current baseline is supplied.
...
- `reclaim:meetings-hours:preview-switch` returns read-only profile-switch comparisons for synthetic hours presets and local profile hints plus a `previewReceipt`. Each profile includes a `timePolicyExplanation` object that uses the same fit/conflict envelope as the dedicated policy explainer.
...
- `reclaim:hours-config:preview-audit` returns the Hours Config audit shape from a synthetic local fixture.
- `reclaim:hours-config:preview-diff` compares two synthetic hours-config snapshots and returns a summary-only drift digest keyed by source handles.
- `reclaim:hours-config:audit` reads configured time schemes from the account and returns summary-only coverage and window counts.
...
- `reclaim:tasks:retarget` reads selected current tasks, derives reviewed date-shifted updates, and applies them only with `--confirm-write`.
