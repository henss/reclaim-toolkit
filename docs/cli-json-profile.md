...
- `reclaim:focus:preview-create` and `reclaim:buffers:preview-create` return preview payloads plus a `previewReceipt` that explains why the surface remains preview-only.
