...
- `reclaim:habits:preview-create`, `reclaim:focus:preview-create`, and `reclaim:buffers:preview-create` return preview payloads plus a `previewReceipt` that explains why each surface remains preview-only. Focus and Buffer previews also emit per-item `timePolicyExplanation` fields when the input fixture includes synthetic policy context.
