...
- Default to high agency inside recoverable bounds; let agents make useful decisions and advance bounded public, paid, creative, venture, tool, distribution, planning, or quality-of-life work when downside is capped and recoverable, and stop for hard-to-recover legal, financial, reputation, security, client/company, credential, irreversible, or safety risk.
...
- Use `durable-workloops` for long-running, multi-step, cross-package, or commit-per-increment agent implementation work when the repo has adopted it or the capability catalog routes to it. If the workflow is missing, stale, confusing, or inefficient, improve the owned `durable-workloops` repo when the fix is safe and generic, or record a concrete follow-up there.
