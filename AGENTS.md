...
- When this repo exposes Agent Atlas surfaces, read `docs/agents/atlas.md` for broad orientation, use `atlas context-pack` for broad or multi-seam tasks, and use `atlas resolve-path` for file-specific tasks before broad search; add or expand atlas metadata only when a concrete task exposes repeated navigation waste or cross-repo context ambiguity.
