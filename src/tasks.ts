...
function selectTimeScheme(
  ...
  if (options.preferredTimePolicyTitle) {
    const exact = schemes.find(
      (scheme) => scheme.title.toLowerCase() === options.preferredTimePolicyTitle?.toLowerCase()
    );
    if (!exact) {
      throw new Error(`Preferred Reclaim time policy title "${options.preferredTimePolicyTitle}" was not found.`);
    }
    return exact;
  }

  return schemes.find((scheme) => scheme.taskCategory === options.eventCategory) ?? schemes[0]!;
}
...
export async function create(
  ...
  const timeSchemeId =
    ...
    client.config.preferredTimePolicyId ??
